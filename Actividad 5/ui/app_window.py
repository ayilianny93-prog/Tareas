...
class AppWindow(tk.Tk):
    ...
    def create_widgets(self) -> None:
        ...
        self.tree = ttk.Treeview(self, columns=("Column1", "Column2"), height= 10)
        self.tree.column("#0", width=275, stretch=True)
        self.tree.column("Column1", width=400, stretch=True, anchor="center")
        self.tree.column("Column2", width=250, stretch=True, anchor="center")

        self.tree.heading("#0", text="Título")
        self.tree.heading("Column1", text="Descripción")
        self.tree.heading("Column2", text="id")
    ...
    def show_table(self):
        for task in self._task_service.get_all_task():
            self.tree.insert("", "end", text= task.title, values= (task.description, task.uuid))
